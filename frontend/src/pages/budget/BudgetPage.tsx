...
const categories = [
  { name: "Transport", amount: 21000, icon: Bus, color: "bg-sky-500" },
  { name: "Stay", amount: 49000, icon: BedDouble, color: "bg-indigo-500" },
  { name: "Activities", amount: 18000, icon: MapPin, color: "bg-amber-500" },
  { name: "Meals", amount: 14000, icon: Utensils, color: "bg-emerald-500" },
];
...
export default function BudgetPage() {
  ...
  const plannedTotal = categories.reduce((total, category) => total + category.amount, 0) + activityCount * 2200;
  const budgetLimit = 124000;
  const remaining = budgetLimit - plannedTotal;
  const dailyBudgets = stops.map((stop, index) => ({ city: stop.city, day: index + 1, amount: 9000 + stop.activities.length * 2200 }));
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="mb-8 grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6"><p className="text-sm font-medium text-slate-500">Budget limit</p><p className="mt-1 text-3xl font-bold text-slate-950">₹{budgetLimit.toLocaleString("en-IN")}</p><p className="mt-2 text-sm text-slate-500">Set for this trip</p></div>
